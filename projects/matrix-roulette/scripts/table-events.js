...
export default class TableEvents {
	...
	constructor(pWorld) {
		...
		// fake results - not working for wheel just for ?server=true
	}
	...
	getBetMap(winningNumber) {
		...
		this.registerBetPlaces.forEach((ele) => {
			playerBetMap.push({id: ele.name, chips: ele.tableEvents.chips})
			if(ele.tableEvents.chips > 0) {
				played.push({id: ele.name, chips: ele.tableEvents.chips})
			}
		})
		return {
			map: playerBetMap,
